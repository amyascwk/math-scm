...
;;; Alternative ordering wrappers

(define (with-left-to-right-alternation thunk)
  (call-with-current-continuation
   (lambda (k)
     (fluid-let ((preprocess-order order-from-left-to-right)
		 (*search-schedule* (empty-search-schedule))
		 (*top-level* k))
       (thunk)))))
...
(define (with-right-to-left-alternation thunk)
  (call-with-current-continuation
   (lambda (k)
     (fluid-let ((preprocess-order order-from-right-to-left)
		 (*search-schedule* (empty-search-schedule))
		 (*top-level* k))
       (thunk)))))
...
(define (with-random-order-alternation thunk)
  (call-with-current-continuation
   (lambda (k)
     (fluid-let ((preprocess-order random-order)
		 (*search-schedule* (empty-search-schedule))
		 (*top-level* k))
       (thunk)))))
...
;;; Alternative ordering mechanisms

(define (order-from-left-to-right alternatives)
  alternatives)

(define (order-from-right-to-left alternatives)
  (reverse alternatives))

(define (random-order alternatives)
  (let ((n (length alternatives)))
    (if (< n 2)
	alternatives
	(let* ((k (random n))
	       (rand-elt (list-ref alternatives k))
	       (alternatives-copy (list-copy alternatives)))
	  (cond ((= k 0) (set! alternatives-copy
			       (list-tail alternatives-copy 1)))
		((= k (- n 1)) (set-cdr! (list-tail alternatives-copy
						    (- k 1))
					 '()))
		(else (set-cdr! (list-tail alternatives-copy (- k 1))
				(list-tail alternatives-copy (+ k
								1)))))
	  (cons rand-elt (random-order alternatives-copy))))))  
...
(define preprocess-order order-from-left-to-right)
